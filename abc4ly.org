...
Gérer une mesure à cheval sur 2 lignes ABC 
	...
	(idée: translate_notes() stocke dans tc le début de la ligne
        convertie (ly_line) et tout le contexte (note, ...). Et il faut
	ajouter une notion de translate_notes_finalize() pour le cas où
        la dernière barre de mesure est manquante.
