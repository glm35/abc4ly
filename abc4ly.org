...
** P0
translate_notes():
	+abc_line = "C2 D2 E2 F2"_+
	+abc_line = ""_+
	+abc_line = "   C2  D2   E2 F2  "_+
	+abc_line = "C2 D2 E2 F  2": légal? => passe avec abc4ly+
	+abc_line = "CDEF GABc"_+
	+M = 6/8 && abc_line = "CDE FGA" (TBC: on veut: default note length != 2)+
	+Séparateur de mesures+
	+c_major+
	+lower_c_major+
	+erreur si octaver = "," et abc_pitch en minuscule+
	+octaver = "'"_+
	+erreur si octaver = "'" et abc_pitch en majuscule+
	state "done" => dump note
	dump note: check that a note has really been setup

Morceau:
	+c_major+
	hello_world & friends

Reconnaître la durée des notes:
...
Exprimer la durée des notes en lilypond:
...
Exprimer la hauteur des notes en lilypond:
